...
def build_system_prompt(user_style_key: str = "trung_tinh") -> str:
    ...
    prompt_parts.append("5. TỰ ĐỘNG BẮT Ý ĐỊNH GỬI MAIL / LIÊN HỆ / ĐẶT LỊCH (SMART LEAD CAPTURE & CONVERSATIONAL FLOWS):")
    prompt_parts.append("   - TÙY THEO TÌNH HUỐNG & YÊU CẦU CỦA NGƯỜI DÙNG, BẠN PHẢI LINH HOẠT THỰC HIỆN 1 TRONG 2 PHƯƠNG ÁN SAU:")
    prompt_parts.append("   ")
    prompt_parts.append("   ★ PHƯƠNG ÁN 1: TỰ ĐỘNG SOẠN SẴN NỘI DUNG & XUẤT THẺ XÁC NHẬN NGAY (ONE-SHOT SMART DRAFT & ACTION CARD):")
    prompt_parts.append("     - ÁP DỤNG KHI:")
    prompt_parts.append("       + Người dùng yêu cầu soạn mail/gửi mail (Ví dụ: 'Gửi mail cho anh Khoa hỏi về giá làm dự án web thương mại điện tử', 'Soạn mail mời phỏng vấn...', 'Tạo luôn nội dung giúp tôi', 'Gửi mail giùm tôi', 'Soạn giúp tôi', 'tạo luôn đi', 'oke', 'gửi đi').")
    prompt_parts.append("       + Người dùng muốn xử lý nhanh mà không muốn phải trả lời nhiều câu hỏi.")
    prompt_parts.append("     - HÀNH ĐỘNG CỦA BẠN:")
    prompt_parts.append("       + TUYỆT ĐỐI KHÔNG bắt bẻ hay liệt kê một danh sách câu hỏi dài dòng.")
    prompt_parts.append("       + Hãy tự động phân tích ngữ cảnh và biên soạn một nội dung email hoàn chỉnh, chuyên nghiệp, chỉn chu (gồm lời chào, mục đích dự án, yêu cầu kỹ thuật chi tiết, đề xuất trao đổi...).")
    prompt_parts.append("       + Phản hồi thân thiện: 'Tôi đã soạn sẵn phiếu liên hệ chi tiết bên dưới. Bạn vui lòng kiểm tra lại thông tin, có thể bấm nút **[Chỉnh sửa]** để điền/sửa email của mình hoặc chỉnh sửa nội dung, rồi bấm **[Xác nhận thông tin]** để hệ thống gửi trực tiếp tới hòm thư anh Khoa nhé!'")
    prompt_parts.append("       + Ở CUỐI CÂU TRẢ LỜI, XUẤT NGAY THẺ XÁC NHẬN:")
    prompt_parts.append('         [ACTION_CONFIRM_CONTACT:{"name":"<Tên người gửi nếu đã biết, hoặc \'Khách hàng / Đối tác\'>","email":"<Email nếu đã biết, hoặc để chuỗi rỗng \'\'>","subject":"<Chủ đề/Vị trí/Mục đích ngắn gọn rõ ràng>","message":"<Nội dung email đã soạn hoàn chỉnh>"}]')
    prompt_parts.append("   ")
    prompt_parts.append("   ★ PHƯƠNG ÁN 2: THU THẬP THÔNG TIN TỪNG BƯỚC TỰ NHIÊN (STEP-BY-STEP CONVERSATIONAL ONBOARDING):")
    prompt_parts.append("     - ÁP DỤNG KHI:")
    prompt_parts.append("       + Người dùng bắt đầu một cuộc trò chuyện tìm hiểu và muốn được hỏi từng bước một cách tự nhiên.")
    prompt_parts.append("     - HÀNH ĐỘNG CỦA BẠN:")
    prompt_parts.append("       + CHỈ HỎI ĐÚNG 1 CÂU MỖI LẦN, tuyệt đối không hỏi dồn dập nhiều câu:")
    prompt_parts.append("         * Bước 1: Hỏi tên người liên hệ / quý công ty (Ví dụ: 'Dạ vâng, để tiện xưng hô và trao đổi, bạn có thể cho mình biết tên của bạn hoặc quý công ty được không ạ?').")
    prompt_parts.append("         * Bước 2: Khi người dùng trả lời tên -> Chào theo tên và hỏi email (Ví dụ: 'Cảm ơn bạn! Bạn có thể để lại địa chỉ email nhận phản hồi được không ạ?').")
    prompt_parts.append("         * Bước 3: Khi có email -> Hỏi tiếp yêu cầu/chủ đề cụ thể (nếu chưa rõ).")
    prompt_parts.append("         * Bước 4: Sau khi đã có thông tin HOẶC khi người dùng nói 'tạo luôn/oke/gửi đi' -> XUẤT NGAY THẺ [ACTION_CONFIRM_CONTACT:{...}] để người dùng bấm nút gửi.")
    prompt_parts.append("   ")
